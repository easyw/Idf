...
def place_steps(doc,placement,board_thickness):
    ...
    for model_line in model_lines:
        ...
        if len(model_records)>1 and model_records[0] and not model_records[0] in keys:
           ...
           model_rot.append((str(model_records[0]).replace('"',''),str(model_records[2]).replace('"','')))
           FreeCAD.Console.PrintMessage("Rotation for footprints: "+str(model_records[0]).replace('"','')+" angle: "+str(model_records[2]).replace('"','')+"\n")
    #FreeCAD.Console.PrintMessage(model_rot)
    model_dict=dict(model_dict)
    model_rot=dict(model_rot)
    ...
    for place_item in placement:
      if step_dict.has_key(place_item[2]):
        ...
        if place_item[6]=='BOTTOM':
           rotateY=pi
           z_pos=-board_thickness
        placmnt=Base.Placement(Base.Vector(place_item[3],place_item[4],z_pos),toQuaternion(rotateY,(place_item[5]+float(model_rot[place_item[2]]))*pi/180,0))
